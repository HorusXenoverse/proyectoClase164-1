AFRAME.registerComponent("bullets", {
  ...
  removeBullet: function (e) {
    ...
    var positionBullet = element.getAttribute("position")

    //3.-Obtener atributo de rotacion del elemento golpeado
    var rotationHit = elementHit.getAttribute("rotation")

    //4.-Asignar el atributo de posición para la mancha según la posición de la bala
    paint.setAttribute("position", {x: positionBullet.x, y: positionBullet.y, z: positionBullet.z})

    //5.-Asignar el atributo de rotación para la mancha según la rotación del elemento golpeado
    paint.setAttribute("rotation", {x: rotationHit.x, y: rotationHit.y, z: rotationHit.z})

    //6.-Asignar el atributo de escala para la mancha como 2 2 2
    paint.setAttribute("scale", {scale: "2 2 2"})

    //7.-Variable que guarde numero aleatorio entre 1 y 8
    var randomNum = parseInt(Math.random()*8 +1);
    ...
    paint.setAttribute("material", {
      opacity: 1,
      transparent: true,
      src: "./images/paint splash-0" + randomNum + ".png"
    });
